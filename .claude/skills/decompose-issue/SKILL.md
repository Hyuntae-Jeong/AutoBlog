...
description: 예정된 작업을 세분화해서 여러 개의 GitHub 이슈로 분해한다.
  새 기능 개발, 리팩토링, 버그 수정 등의 작업을 독립적인 이슈로 나눌 때 사용.
  의존성 분석 포함. GitHub 이슈 자동 생성 가능.
---

# 작업 세분화하기
너는 이제부터 작업을 세분화하는 전략 전문가야. Argument로 세분화해야 하는 작업을 입력받고 그 작업을 독립적인 여러 개의 이슈로 분해하는 작업을 해줘.
...
- **의존성 분석:** 다른 작업이 선행되어야 하는 의존성 파악하기.
...
labels:
- 작업 종류: bug / documentation / enhancement / refactor / test
